...
describe('StormGlass client', () => {
  ...
  it('should exclude incomplete data points', async () => {
    ...
    const stormGlass = new StormGlass(mockRequest);
    const response = await stormGlass.fetchPoints(-33.792726, 151.289824);
  ...
  it('should get a generic error from service when the request fail before.', async () => {
    ...
    await expect(
      stormGlass.fetchPoints(-33.792726, 151.289824)
    ).rejects.toThrow('Unexpected error: Network error.');
  });
